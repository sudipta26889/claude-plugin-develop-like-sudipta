...
if [ "$EXTENSION" = "py" ]; then
  for pattern in "test_${NAME_NO_EXT}.py" "${NAME_NO_EXT}_test.py"; do
    ...
    for dir in "$DIRNAME" "$DIRNAME/tests" "$(git rev-parse --show-toplevel 2>/dev/null)/tests"; do
...
if [ "$EXTENSION" = "ts" ] || [ "$EXTENSION" = "tsx" ] || [ "$EXTENSION" = "js" ] || [ "$EXTENSION" = "jsx" ]; then
  for suffix in "test" "spec"; do
    for dir in "$DIRNAME" "$DIRNAME/__tests__" "$(git rev-parse --show-toplevel 2>/dev/null)/tests"; do
...
if [ "$TEST_EXISTS" = false ]; then
  ...
  cat <<EOF
{
  "additionalContext": "[TDD GATE] ⚠️ No test file found for '${BASENAME}'. Pillar 5 requires: write a FAILING test first (RED phase) before editing production code. Create test_${NAME_NO_EXT}.py (or equivalent) with the expected behavior, verify it FAILS, then proceed with this edit."
}
EOF
fi
